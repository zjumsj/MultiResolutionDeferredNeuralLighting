...
def sample_texture_lod(neural_texture, uv, lod, args):
    if not args.mipmap:
        args.logger.info("mipmap=Flase not supported")

    sampled_texture = sample_texture_mipmap_lod(neural_texture,uv,lod)
    return sampled_texture

"""
Internal implementation
"""
# [0,1] -> HW correspond to mipmap level
...
def _sample_texture(texture, indices, mode='bilinear', resolution=None):
    ...
    if mode == 'nearest':
        ...
        X1 = tf.minimum(tf.ceil(X), tf.cast(resolution[0]-1,tf.float32))
        Y0 = tf.maximum(tf.floor(Y), 0)
        Y1 = tf.minimum(tf.ceil(Y), tf.cast(resolution[1]-1,tf.float32))
...
def sample_texture_single(texture, uvs, mode = 'bilinear'):
    ...
    for i in range(batch_size):
        result = _sample_texture(texture, indices[i], mode, tf.shape(texture)[0:2])
...
def sample_texture_hierarchy_mipmap(textures, uvs):
    ...
    levels = len(textures)
    if levels == 1:
        return sample_texture_single(textures[0], uvs, mode="bilinear")
    ...
    # notice last row & column is zero
    # https://www.tensorflow.org/api_docs/python/tf/image/image_gradients
...
def sample_texture_mipmap_lod(textures,uvs,lod):
